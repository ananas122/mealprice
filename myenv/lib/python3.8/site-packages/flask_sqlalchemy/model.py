...
class BindMetaMixin(type):
    ...
    def __init__(
        ...
        if "metadata" not in cls.__dict__ and "__table__" not in cls.__dict__:
...
class BindMixin:
    ...
    def __init_subclass__(cls: t.Type[BindMixin], **kwargs: t.Dict[str, t.Any]) -> None:
        if (
            "metadata" not in cls.__dict__
            and "__table__" not in cls.__dict__
            and hasattr(cls, "__bind_key__")
        ):
            bind_key = getattr(cls, "__bind_key__", None)
            parent_metadata = getattr(cls, "metadata", None)
            metadata = cls.__fsa__._make_metadata(bind_key)
...
class NameMetaMixin(type):
    ...
    def __table_cls__(self, *args: t.Any, **kwargs: t.Any) -> sa.Table | None:
            ...
            and (
                isinstance(
                    ...
                    (
                        ...
                        sa_orm.decl_api.DeclarativeAttributeIntercept,
                    ),
                )
